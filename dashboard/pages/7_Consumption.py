...
if not pop_df.empty:
    ...
    clean_pop_df = pop_df.copy()
    
    st.dataframe(
        clean_pop_df[["country_name", "total_gen_gwh", "total_cons_gwh", "population_count", "gen_kwh_per_capita", "cons_kwh_per_capita"]].rename(columns={
            ...
            "Total Generation (GWh)": lambda x: f"{x:,.1f}" if pd.notnull(x) else "N/A",
            "Total Consumption (GWh)": lambda x: f"{x:,.1f}" if pd.notnull(x) else "N/A",
            "Population": lambda x: f"{x:,.0f}" if pd.notnull(x) else "N/A",
            "Generation per Capita (kWh)": lambda x: f"{x:,.0f}" if pd.notnull(x) else "N/A",
            "Consumption per Capita (kWh)": lambda x: f"{x:,.0f}" if pd.notnull(x) else "N/A"
        }),
    ...
    with col1:
        chart_df1 = clean_pop_df.dropna(subset=["cons_kwh_per_capita"])
        if not chart_df1.empty:
            fig_c1 = px.bar(
                chart_df1, x="country_name", y="cons_kwh_per_capita",
    ...
    with col2:
        chart_df2 = clean_pop_df.dropna(subset=["gen_kwh_per_capita"])
        if not chart_df2.empty:
            fig_c2 = px.bar(
                chart_df2, x="country_name", y="gen_kwh_per_capita",
